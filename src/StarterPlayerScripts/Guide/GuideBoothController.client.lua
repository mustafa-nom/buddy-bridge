...
local function ensureDisplay()
	if state.Display then return end
	...
	for i = 1, 3 do
		local label = UIStyle.MakeLabel({
			Size = UDim2.fromScale(0.21, 0.86),
			Text = ("Slot %d\nEmpty"):format(i),
...
local function makeRow(parent: Frame, yScale: number): Frame
	local row = Instance.new("Frame")
	row.Size = UDim2.fromScale(0.94, 0.16)
	row.Position = UDim2.fromScale(0.03, yScale)
...
local function openPicker(payload)
	...
	local _ = title
	local colorRow = makeRow(state.Picker, 0.22)
	...
	local shapeRow = makeRow(state.Picker, 0.43)
	...
	local confirm = UIStyle.MakeButton({
		Size = UDim2.fromScale(0.44, 0.16),
