	...
	function slot:Fill(tool: Tool): ...any
		...
		local function assignToolData(): ()
			...
			if tool:GetAttribute("Phish3DIcon") then
				...
				vp.LightColor = Color3.fromRGB(255, 240, 200)
				vp.ZIndex = ToolIcon.ZIndex + 1
				vp.Parent = SlotFrame

				local container = Instance.new("Model")
				container.Name = "PreviewModel"
				...
				for _, p in ipairs(container:GetDescendants()) do
					if p:IsA("BasePart") then
						...
						p.CanQuery = false
						p.CanTouch = false
					elseif p:IsA("Script") or p:IsA("LocalScript")
						or p:IsA("Sound") or p:IsA("ParticleEmitter")
					then
						p:Destroy()
					end
				end
