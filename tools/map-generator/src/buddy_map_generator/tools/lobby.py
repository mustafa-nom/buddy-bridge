...
def emit_lobby_lua(
    *,
    ...
    ox, oy, oz = origin
    # idempotency: nuke any existing lobby + the studio-default baseplate / spawn
    # so re-running on a fresh place is one call instead of three.
    ...
    # ----- decorative pass: bushes, benches, flower bed
    p.line(make_model("decor", parent="lobby", name="Decor"))

    # bushes ring the perimeter so the grass field doesn't read as empty
    bush_positions = [
        (-90, -90), (-60, -95), (60, -95), (90, -90),
        (-95, -30), (-95, 30), (95, -30), (95, 30),
        (-90, 90), (-30, 95), (30, 95), (90, 90),
    ]
    for i, (bx, bz) in enumerate(bush_positions):
        p.line(
            make_part(
                f"bush_{i}",
                parent="decor",
                name=f"Bush{i}",
                size=(5, 4, 5),
                cframe=cframe_pos(ox + bx, oy + 2, oz + bz),
                color_rgb=PALETTE.treehouse_leaf,
                material_name="Grass",
                shape="Ball",
            )
    ...
    # park benches flanking the welcome path so the lobby feels lived in
    bench_specs = [
        (-12, -25, 0),
        (12, -25, 0),
        (-12, 5, 0),
    ...
    flower_colors = [
        PALETTE.capsule_a,  # warm orange
        PALETTE.capsule_b,  # soft blue
        PALETTE.capsule_c,  # soft purple
        PALETTE.capsule_d,  # spring green
    ]
