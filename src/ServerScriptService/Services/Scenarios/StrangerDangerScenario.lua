--!strict
-- Generates Stranger Danger Park NPCs for the Guide booth accusation loop.
...
local function gatherSpawnPoints(levelModel: Model)
	...
	for _, part in ipairs(TagQueries.GetTaggedInside(levelModel, PlayAreaConfig.Tags.BuddyNpcSpawn)) do
		if part:IsA("BasePart") then
			...
			table.insert(result, { Id = id, Anchor = anchor })
		end
	end
	return result
end
...
function StrangerDangerScenario.Generate(levelModel: Model): any?
	...
	for i, spawn in ipairs(spawns) do
		...
		table.insert(npcs, {
			...
			Archetype = archetype,
			Role = role,
			Silhouette = archetypeData and archetypeData.Silhouette,
			Cue = cues[1],
			Cues = cues,
			Verdict = role == ScenarioTypes.NpcRoles.Risky and StrangerDangerLogic.Verdict.Avoid
